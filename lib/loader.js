...
function loader(src) {
  ...
  if (/\.jsx?$/.test(issuer)) {
    ...
    Promise.resolve(String(src)).then(reactSvgCore.optimize(loaderOpts.svgo)).then(reactSvgCore.transform({ jsx: false })).then(({ code }) => babel.transform(code)).then(result => cb(null, result.code)).catch(err => cb(err));
  }
